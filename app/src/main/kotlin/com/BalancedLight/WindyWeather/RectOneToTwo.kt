...
class RectOneToTwo(context: Context?, strName: String?) {
    private val indexBuffer: ShortBuffer
    private val mName: String?
    private val textureBuffer: FloatBuffer
    private val vertexBuffer: FloatBuffer
    private var mbTextureLoaded = false
    private var mLastResolvedTextureId = 0
    private var mLastBlur = false
    private val textures = IntArray(1)
    private val vertices =
        floatArrayOf(-2.0f, -1.0f, 0.0f, 2.0f, -1.0f, 0.0f, 2.0f, 1.0f, 0.0f, -2.0f, 1.0f, 0.0f)
    ...
    fun deleteGLTexture(gl: GL10?, context: Context?) {
        if (this.mbTextureLoaded && gl != null) {
            gl.glDeleteTextures(1, this.textures, 0)
            this.mbTextureLoaded = false
            this.textures[0] = 0
        }
    ...
    fun loadGLTexture(gl: GL10?, context: Context?, texture_id: Int, bBlur: Boolean) {
        ...
        if (this.mbTextureLoaded && resolvedTextureId == this.mLastResolvedTextureId && bBlur == this.mLastBlur) {
            return
        }
        if (this.mbTextureLoaded) {
            gl.glDeleteTextures(1, this.textures, 0)
            this.mbTextureLoaded = false
            this.textures[0] = 0
        }
